...
@MainActor final class BotMessageActionsTests: XCTestCase {
    ...
    func testPromptMenuOffersTheTapbackRowAboveCopyAndNoRemoveWithoutAReaction() throws {
        var picked: [String?] = []
        let reacting = BotMessageActions.Reacting(current: nil, isEnabled: true, react: { picked.append($0) })
    ...
    func testPromptMenuHighlightsYourReactionAndOffersRemove() throws {
        ...
        let reacting = BotMessageActions.Reacting(current: "❤️", isEnabled: true, react: { picked = [$0] })
        let items = BotMessageActions.items(copyText: "Ping", isHapticsEnabled: false, reacting: reacting, copy: { _ in })
    ...
    func testTapbacksStayInertWhileAWriteIsInFlight() {
        let reacting = BotMessageActions.Reacting(current: "👍", isEnabled: false, react: { _ in })
        let items = BotMessageActions.items(copyText: "Ping", isHapticsEnabled: false, reacting: reacting, copy: { _ in })

        XCTAssertEqual(items.filter { !$0.isEnabled }.count, 7)
        XCTAssertTrue(items.first { $0.kind == .copy }?.isEnabled == true)
    }
